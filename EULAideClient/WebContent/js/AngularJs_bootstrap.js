...
myApp.controller('fileCtrl', ['$scope', '$http', '$sce', function($scope, $http, $sce){
	...
	$scope.generate = function(){
		...
		if(lastInput === "File"){
			...
			//var uploadUrl = 'http://butterbur17.iai.uni-bonn.de:8081/eula/fileUploadJSON';			
			...
			//var uploadUrl = 'http://butterbur17.iai.uni-bonn.de:8081/eula/urlUploadJSON';
	...
	var summaryExtraction = function(summary, annotType){
		...
		{			
			...
			var annotObj = {
					...
					id : undefined,
					action : undefined,
					annotationSentence: undefined,
					//advancedAction : undefined,
					relatedAnnotId : undefined,
					relatedAnnotType : undefined
			}
			var jsonObj = angular.fromJson(annot);			
			var str = jsonObj.annotation;
			//var newStr = JSON.stringify(str,null,'\t').replace(/\n/g,'<br />').replace(/\t/g,'&nbsp;&nbsp;&nbsp;');					

			annotObj.annotation = str;
			annotObj.id =  jsonObj.id;
			if(jsonObj.hasOwnProperty("action"))
				annotObj.action = jsonObj.action;
			
			if (jsonObj.hasOwnProperty("annotationSentence"))
				annotObj.annotationSentence = jsonObj.annotationSentence;
/*
